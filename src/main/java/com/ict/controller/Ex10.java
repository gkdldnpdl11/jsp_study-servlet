...
public class Ex10 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 페이지 이동 (리다이렉트, 포워딩)
		// => 현재 페이지에서 작업을 한 후 다른 페이지로 이동 (클라이언트에게는 현재 페이지가 보이지 않는다)
		
		// 리다이렉트 : 다른 페이지로 이동하면서 기존의 request, response 정보가 사라진다
		// 					즉, request 안에 있는 파라미터도 사라진다
		//					새로운 request, response 가 만들어진다
		//					주소창에 최종 주소가 보인다
		//					response.sendRedirect("주소");
		// response.sendRedirect("Ex10_2"); => 확장자가 없으면 servlet 으로 이동
		// response.sendRedirect("view/Ex09.jsp"); => html, jsp 로 이동
		
		// 포워드 : 다른 페이지로 이동하면서 기존의 request, response 의 정보를 가지고간다
		//				파라미터도 사용 가능하다
		// 				주소창에는 최초 주소가 보인다
