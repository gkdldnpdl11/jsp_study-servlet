...
public class Ex05 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 클라이언트가 서버에게 정보를 요청하면 필요한 정보를 보낼 수 있다
		// 요청을 처리하기 위해서 함께 전달되는 정보를 파라미터(parameter)라고 한다
		
		// 파라미터의 자료형은 기본적으로 String 이거나 String [] (같은 이름이 여러개이면 배열로 넘어옴)이다
		
		// 클라이언트가 요청할 때 사용하는 메소드는 get 방식, post 방식 이다
		// get 방식 : 주소창에 정보가 노출된다 (정보를 헤더에 담아서 보냄) (url~~?name=value&name=value~~)
		// post 방식 : 주소창에 정보가 노출되지 않는다 (정보를 바디에 담아서 보냄)
		...
		// 만약에 s1이라는 이름으로 숫자, s2라는 이름으로 숫자, op라는 이름으로 +, -, *, / 중 하나가 넘어온다고 가정
